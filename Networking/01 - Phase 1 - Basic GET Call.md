...
```kotlin
// Creates and shares the Retrofit setup for the app.  
object NetworkClient {  
    // Base URL for the sample JSON API used in Phase 1.  
    private const val BASE_URL = "https://jsonplaceholder.typicode.com/"  
  
    // Logs request and response details to help you learn the network flow.  
    private val loggingInterceptor = HttpLoggingInterceptor().apply {  
        level = HttpLoggingInterceptor.Level.BODY  
    }  
  
    // OkHttp handles the actual HTTP connection and adds interceptors.  
    private val okHttpClient = OkHttpClient.Builder()  
        .addInterceptor(loggingInterceptor)  
        .build()  
  
    // Retrofit converts the HTTP response into Kotlin objects.  
    private val retrofit = Retrofit.Builder()  
        .baseUrl(BASE_URL)  
        .client(okHttpClient)  
    ...
    // The concrete API implementation generated by Retrofit.  
